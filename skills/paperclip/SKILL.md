...
- **Route human decisions to the root requester.** When assigning a human decision/sign-off issue, first use `rootHumanRequester.userId` from `/api/issues/{id}/heartbeat-context` as the attribution hint, then validate/fallback according to the board-user routing rules. If absent, fetch parent issues by `parentId` and use the rootmost ancestor with `createdByUserId`, then the triggering board comment's `authorUserId`, then the current issue's `createdByUserId`; only fall back to a triage/default board user when no requester can be resolved.
