...
export function resolveExecutionWorkspaceEnvironmentId(input: {
  ...
  const assigneeIntended: { environmentId: string; source: Exclude<ExecutionWorkspaceEnvironmentSource, "workspace"> } = (() => {
    if (input.agentDefaultEnvironmentId) {
      return { environmentId: input.agentDefaultEnvironmentId, source: "agent" as const };
    }
    if (input.instanceDefaultEnvironmentId) {
      return { environmentId: input.instanceDefaultEnvironmentId, source: "instance" as const };
    }
    return { environmentId: input.localDefaultEnvironmentId, source: "default" as const };
  })();

  // PAPA-380 / PAPA-431: when reusing a workspace, detect environment mismatch
  // and refuse silent reuse so the caller can force a fresh realization.
  if (input.workspaceConfig?.environmentId !== undefined) {
    const workspaceEnvironmentId =
      input.workspaceConfig.environmentId ?? input.localDefaultEnvironmentId;
